...
def preprocess_power(df):
	...
	t = offset
	start_time = datetime.fromtimestamp(offset)
	actual_start = datetime.combine(start_time.date(), time(0,0))
	start_ind = int(datetime.timestamp(start_time) - datetime.timestamp(actual_start))
	power[start_ind] = df.iloc[0,0]
	for i in range(1,df.shape[0]):
		if int(df.iloc[i,1]) != t:
			if round(df.iloc[i,1]-t) == 1.0:
				power[start_ind+t+1-offset] = df.iloc[i,0]
				t+=1			
				...
				for j in range(int(df.iloc[i,1]-orig_t)):
					power[start_ind+req_offset+j] = avg
					t+=1
			else:
				orig_t = t
				req_offset = orig_t+1-offset
				for j in range(int(df.iloc[i,1]-orig_t)):
					power[start_ind+req_offset+j] = 0
					t+=1
		else: 
			power[start_ind+t-offset] = (power[start_ind+t-offset]+df.iloc[i,0])/2
	return power
